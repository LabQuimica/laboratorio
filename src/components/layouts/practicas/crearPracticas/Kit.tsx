        ...
        <Card key={kit.id_item} className="flex flex-col">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-start">
              ...
              <Badge variant={kit.status ? "secondary" : "destructive"}>
                {kit.status ? "Disponible" : "Agotado"}
              </Badge>
            </div>
          </CardHeader>
                  ...
                  {kit.contenido_kit.length > maxChars && (
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          variant="link"
                          className="text-xs mt-1 bg-btn-details text-text-details"
                        >
                          Ver más
                        </Button>
                      </DialogTrigger>
          ...
          <div className="pb-3 px-4 mt-auto space-y-2">
            <div className="text-xs text-right text-muted-foreground mt-4">
              Actualizado:{" "}
              {new Date(kit.fecha_modificacion).toLocaleDateString()}
            </div>
