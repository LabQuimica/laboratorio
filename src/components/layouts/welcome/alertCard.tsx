...
const ItemCard: React.FC<{ item: ItemProps }> = ({ item }) => {
  ...
  return (
    <div
      className={`rounded-lg overflow-hidden border ${statusInfo.borderColor} shadow-sm hover:shadow-md transition-shadow duration-200 w-full h-full flex flex-col dark:shadow-neutral-900`}
    >
      <div className={`p-4 ${statusInfo.bgColor} flex-1 flex flex-col`}>
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center">
            ...
            <h3
              className={`ml-2 text-lg font-bold ${statusInfo.textColor} line-clamp-2`}
            >
              {item.nombre}
            </h3>
          </div>
...
const AlertaInventario: React.FC<{ items: ItemProps[] }> = ({ items }) => {
  const itemsBajoStock = items.filter(
    (item) => (item.cantidad / item.cantidad_maxima) * 100 < 30
  );
        ...
        <>
          <div className="bg-red-50 dark:bg-red-950/30 border-l-4 border-red-500 p-4 rounded-md">
            ...
            <p className="text-neutral-700 dark:text-neutral-300 mt-2 ml-7">
              Los siguientes artículos tienen menos del 30% de su capacidad
              disponible:
            </p>
          </div>
